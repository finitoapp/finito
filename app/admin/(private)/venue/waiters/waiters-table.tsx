...
const createColumns = (t: TFunction): ColumnDef<Task>[] => [
	{
		accessorKey: "name",
		header: createSortableHeader(t("waiters:table.columns.name")),
		cell: ({ row }) => (
			<Link
				href={
					`/admin/venue/waiters/detail?id=${encodeURIComponent(row.original.id)}` as never
				}
			>
	...
	},
	{
		...
		header: createSortableHeader(t("waiters:table.columns.device-name")),
		cell: ({ row }) =>
...
const createFilterableColumns = (t: TFunction) =>
	[
		{
			id: "name",
			title: t("waiters:table.columns.name"),
		},
...
export function WaitersTable() {
	...
	const columnVisibilityDriver = useDataTableVisibilityDriver("waiters");
		...
		() =>
			({ filters, sorting, setData, pagination: { limit, cursor } }) => {
				...
				const query = createQuery((db) => {
					let qb = db
						.selectFrom("waiter")
						.leftJoin("device", "waiter.deviceId", "device.id")
						.select([
							"waiter.id as id",
						...
						qb = qb.where((eb) =>
							eb.or([
								...
								eb.and([
									...
									eb("waiter.id", "<", previousCursor.id as never),
								]),
							]),
						);
					}
					...
					qb = qb
						...
						.orderBy("waiter.id", "desc");

					for (const filter of filters) {
						if (filter.id === "name") {
							qb = qb.where("waiter.name", "like", `${filter.value}%` as never);
						}
					}
